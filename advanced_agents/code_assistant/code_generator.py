...
class CodeSolution(BaseModel):
    ...
    assumptions: Optional[str] = Field(description="Any assumptions made about the requirements")
...
class CodeGenerator:
    def __init__(self):
        ...
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert Python programmer. Generate clean, efficient, and well-commented Python code.
...
{format_instructions}"""),
            ("human", """Create Python code for the following request:
...
Generate complete, runnable code that solves this request. Include proper imports, error handling, and comments.""")
    ...
    def _extract_code_from_response(self, raw_response: str, user_request: str) -> CodeSolution:
        ...
        if json_match:
            ...
            try:
                code_match = re.search(r'"code":\s*"(.*?)"(?=,\s*"explanation")', json_content, re.DOTALL)
                code = ""
                if code_match:
                    code_str = code_match.group(1)
                    code = code_str.encode().decode('unicode_escape')
                
                explanation_match = re.search(r'"explanation":\s*"(.*?)"(?=,\s*"imports")', json_content, re.DOTALL)
                explanation = explanation_match.group(1) if explanation_match else f"Generated solution for: {user_request}"
                
                imports_match = re.search(r'"imports":\s*\[(.*?)\]', json_content, re.DOTALL)
                imports = []
                if imports_match:
                ...
                assumptions_match = re.search(r'"assumptions":\s*"(.*?)"(?=\s*})', json_content, re.DOTALL)
    ...
    def _create_fallback_solution(self, raw_response: str, user_request: str) -> CodeSolution:
