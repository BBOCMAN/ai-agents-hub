...
class ValidationResult(BaseModel):
    ...
    suggestions: List[str] = Field(
        description="AI-powered suggestions for fixing the code"
    )
...
class CodeValidator:
    ...
    def __init__(self, timeout: int = 30, max_output_size: int = 10000):
        ...
        try:
            self.llm = get_llm()
            self.validation_parser = PydanticOutputParser(
        ...
        self.error_analysis_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """You are an expert Python debugger. Analyze the given code and error to provide helpful suggestions.
...
Analyze this error and provide specific suggestions for fixing the code.""",
        ...
        for import_line in import_lines:
            try:
                ...
                if import_line.startswith("from "):
                    module_name = (
                        import_line.split(" import ")[0].replace("from ", "").strip()
                    )
                else:
                    module_name = (
                        import_line.replace("import ", "").split(".")[0].strip()
                    )
                ...
                if (
                    module_name in self.builtin_modules
                    or module_name in self.safe_imports
                ):
    ...
    def analyze_error(
        self, code: str, error_type: str, error_message: str, full_traceback: str = ""
    ) -> ValidationResult:
    ...
    def comprehensive_validation(self, code: str) -> ValidationResult:
        ...
        if not syntax_valid:
            if self.ai_enabled:
                return self.analyze_error(
                    code, "SyntaxError", "; ".join(syntax_messages)
                )
            else:
                return ValidationResult(
                    is_valid=False,
        ...
        if not import_result.all_imports_valid:
            ...
            if import_result.missing_packages:
                suggestions.append(
                    f"Install missing packages: {', '.join(import_result.missing_packages)}"
                )
            if import_result.invalid_imports:
                suggestions.extend(
                    [f"Fix import: {imp}" for imp in import_result.invalid_imports]
                )
            ...
            if self.ai_enabled:
                return self.analyze_error(
                    code,
                    "ImportError",
                    f"Invalid imports: {import_result.invalid_imports}",
                )
            else:
                return ValidationResult(
                    is_valid=False,
        ...
        if execution_result.success:
            ...
            if self.ai_enabled:
                return self.analyze_error(
                    code, "RuntimeError", execution_result.error, execution_result.error
                )
            else:
                return ValidationResult(
                    is_valid=False,
