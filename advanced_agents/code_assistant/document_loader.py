...
class DocumentProcessor:
    ...
    def __init__(self, chunk_size: int = 600, chunk_overlap: int = 50):
        ...
        try:
            ...
            logger.error(
                "❌ Error initializing embeddings. Make sure your GOOGLE_API_KEY is set correctly."
            )
    ...
    def load_documents(self, doc_paths: List[str] = None) -> List[Document]:
        ...
        logger.info(
            f"✅ Processed {len(documents)} files into {len(self.documents)} chunks"
        )
    ...
    def create_vector_store(self, save_path: str = "vector_store") -> FAISS:
        ...
        if not self.embeddings:
            raise ValueError(
                "Embeddings are not initialized. Check your GOOGLE_API_KEY."
            )
    ...
    def get_or_create_vector_store(self, save_path: str = "vector_store") -> FAISS:
        ...
        if (vector_store_path / "index.faiss").exists() and (
    ...
    def search_relevant_docs(
        self, query: str, k: int = 4, filter_by_type: str = None
        ...
        try:
            ...
            logger.info(
                f"🔍 Search completed: Found {len(relevant_docs)} relevant documents for '{query}'"
            )
...
if __name__ == "__main__":
    ...
    if processor.embeddings:
        ...
        if docs:
            print(
                f"✅ Successfully loaded {len(docs)} document chunks from {len(processor.file_metadata)} files"
            )
            ...
            print(
                "context, results = processor.search_relevant_docs('your query here')"
            )
            print("```")
        else:
            print(
                "❌ No documents were loaded. Please check if documentation files exist."
            )
    else:
