...
class CodeAssistantAgent:
    ...
    def _retrieve_context(self, state: CodeAssistantState) -> CodeAssistantState:
        ...
        try:
            context, source_docs = self.doc_processor.search_relevant_docs(state.user_request)
    ...
    def _generate_code(self, state: CodeAssistantState) -> CodeAssistantState:
        ...
        try:
            ...
            if state.error_history:
                enhanced_request += f"\n\nPREVIOUS ERRORS TO FIX:\n" + "\n".join(state.error_history[-2:])
            
            # Generate code solution
            code_solution = self.code_generator.generate_code(enhanced_request, state.context)
    ...
    def _validate_code(self, state: CodeAssistantState) -> CodeAssistantState:
        ...
        try:
            if state.code_solution:
                validation_result = self.validator.execute_code_safely(state.code_solution.code)
                ...
                if validation_result.success:
                    ...
                    logger.warning(f"⚠️ Code validation failed: {validation_result.error}")
                    # Add error to history for next iteration
                    error_msg = f"Iteration {state.iteration_count + 1}: {validation_result.error}"
                    if hasattr(validation_result, 'syntax_errors') and validation_result.syntax_errors:
                        error_msg += f" | Syntax: {validation_result.syntax_errors}"
                    if hasattr(validation_result, 'missing_imports') and validation_result.missing_imports:
                        error_msg += f" | Missing imports: {validation_result.missing_imports}"
    ...
    def _iterate_correction(self, state: CodeAssistantState) -> CodeAssistantState:
        ...
        logger.info(f"🔄 Preparing iteration {state.iteration_count + 1} with error corrections...")
    ...
    def process_request(self, user_request: str, max_iterations: int = 3) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result = {
                ...
                "source_documents": [doc.metadata.get('source', 'Unknown') for doc in final_state.source_docs],
